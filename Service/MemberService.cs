...
namespace Infrastructure
{
    public class MemberService : IMemberService
    {
       ...
       public async  Task<MemberDto> GetMemberByCode(string memberCode)
        {
             var mbr = (await repo.GetAllWitAllIncludeAsync(x => x.MemberCode == memberCode)).FirstOrDefault();
            return _mapper.Map<MemberDto>(mbr);
        }

       public Task<IEnumerable<MemberDto>> GetMemberByType(string memberType)
        {
